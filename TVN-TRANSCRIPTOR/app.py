...
async def home(request: Request):
    return templates.TemplateResponse(request=request, name="index.html", context={"titulo": APP_NAME})
...
async def recibir_transcripcion(data: WebhookResultado):
    if data.task_id in TAREAS_PENDIENTES:
        ...
        TAREAS_PROCESADAS[data.task_id] = {
            "nombre": info["nombre"],
            "texto": data.texto,
            "segmentos": [seg.dict() for seg in data.segmentos]
        }
...
async def check_status(task_id: str):
    if task_id in TAREAS_PROCESADAS:
        return {"estado": "completado", "texto": TAREAS_PROCESADAS[task_id]["texto"], "segmentos": TAREAS_PROCESADAS[task_id]["segmentos"]}
    if task_id in TAREAS_PENDIENTES:
        return {"estado": TAREAS_PENDIENTES[task_id]["estado"], "texto": "", "segmentos": []}
